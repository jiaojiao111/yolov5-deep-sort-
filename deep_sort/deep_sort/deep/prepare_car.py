...
PATH_ALL_IMAGES = PATH_NEW_DATASET + '/all_images'  # 将所有的数据放在all_images文件夹
PATH_TRAIN = PATH_NEW_DATASET + '/train' # 然后再划分数据集，train
PATH_TEST = PATH_NEW_DATASET + '/test' # 用作test
...
class BatchRename():
    ...
    def rename(self):
    # 重命名所有图片，并且放入不同的文件夹下，相同的车辆创建一个目录
        for aroot, dirs, files in os.walk(self.path):
            # aroot是self.path目录下的所有子目录（含self.path）,dir是self.path下所有的文件夹的列表.
            filelist = files  # 注意此处仅是该路径下的其中一个列表
            # print('list', list)
            ...
            for i, dir in enumerate(dirs):
                for root, _, files in os.walk(images_path + '/' + dir, topdown=True):
                    for j, file in enumerate(files):
                        #遍历所有图片
                        if(j==0): # test dataset；每个车辆的第一幅图片归为测试集合，其他的归为训练集
...
if __name__ == '__main__':
    demo = BatchRename() #实例化一个类，然后做resize，rename，split
    demo.resize()
    demo.rename()
    demo.split()
