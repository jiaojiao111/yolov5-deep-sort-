...
class KalmanFilter(object):
    ...
    8维状态空间[x, y, a, h, vx, vy, va, vh]包含边界框中心位置(x, y)，纵横比a，高度h和它们各自的速度，求一阶导数。
    ...
    def __init__(self):
        ...
        self._motion_mat = np.eye(2 * ndim, 2 * ndim) ##构造了一个F的状态转移矩阵8*8
        for i in range(ndim):
            self._motion_mat[i, ndim + i] = dt
        self._update_mat = np.eye(ndim, 2 * ndim) ##H测量矩阵，4*8（投影矩阵）
    ...
    def initiate(self, measurement):
        ...
        mean_pos = measurement #一个1*4的xyah
        mean_vel = np.zeros_like(mean_pos) #初始化0，mean_vel：1*4
        # Translates slice objects to concatenation along the first axis
        mean = np.r_[mean_pos, mean_vel] #np.r_拼接：1*8

        # 由测量初始化均值向量（8维）和协方差矩阵（8x8维）
        std = [
            2 * self._std_weight_position * measurement[3],#引入_std_weight_position对measurement进行一定的加权
        ...
        return mean, covariance #mean：1*8，covariance:8*8
    ...
    def predict(self, mean, covariance):
        ...
        motion_cov = np.diag(np.square(np.r_[std_pos, std_vel])) ##8*8，拼接std_pos和std_vel
        ...
        return mean, covariance #t时刻的，mean：1*8，covariance：8*8


    def project(self, mean, covariance):
        """Project state distribution to measurement space.
        投影状态分布到测量空间（正态分布在状态空间，需要投影）
       ...
       返回（ndarray，ndarray），返回给定状态估计的投影平均值和协方差矩阵（投影后）
        ...
        # R为测量过程中噪声的协方差；初始化噪声矩阵R（4*4）
        innovation_cov = np.diag(np.square(std))

        # 将均值向量映射到检测空间，即 Hx'
        mean = np.dot(self._update_mat, mean)#Hx'
        # 将协方差矩阵映射到检测空间，即 HP'H^T
        covariance = np.linalg.multi_dot((
            self._update_mat, covariance, self._update_mat.T))#HP'H^T
        return mean, covariance + innovation_cov # 公式(4)，S：covariance + innovation_cov
